...
class App extends React.Component {
 ...
 updateNominatedResults = (movies) => {
  let nominated = {}
  this.state.nominations.forEach(
   (nominee) => (nominated[nominee.imdbID] = true)
  )
  return movies.map((movie) =>
   nominated[movie.imdbID] ? { ...movie, nominated: true } : movie
  )
 }
 ...
 componentDidUpdate(prevState) {
  if (prevState.search !== this.state.search && this.state.searching === true) {
   if (this.state.search === '') {
    ...
    axios
     .get(`https://www.omdbapi.com/?s=${this.state.search}&apikey=${API_KEY}`)
     .then((movies) => {
      if (movies.data.Search) {
       this.setState({
        results: this.updateNominatedResults(movies.data.Search),
        searching: false,
       })
      }
     })
